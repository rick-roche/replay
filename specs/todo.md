# Re:Play — Completion Checklist

See stories in [user-stories.md](./user-stories.md) and [feature-spec.md](./feature-spec.md) for project context.

## Epic 1: User Authentication & Account Setup

- [x] Story 1.1 — Login with Spotify
- [ ] Story 1.2 — Persist User Session

## Epic 2: External Data Source Configuration

- [x] Story 2.1 — Configure Last.fm Profile
- [x] Story 2.2 — Configure Discogs Profile
- [x] Story 2.3 — Configure Setlist.fm Profile

## Epic 3: Data Source Selection & Filtering

- [x] Story 3.1 — Select Data Source
- [x] Story 3.2 — Filter Last.fm Data
- [x] Story 3.3 — Filter Discogs Collection
- [x] Story 3.4 — Filter Setlist.fm Concerts

## Epic 4: Presets

- [ ] Story 4.1 — Use Presets
- [ ] Story 4.2 — Preset Transparency

## Epic 5: Data Fetching & Normalisation

- [x] Story 5.1 — Fetch Source Data
- [x] Story 5.2 — Normalise Source Data

## Epic 6: Spotify Matching Engine

- [x] Story 6.1 — Match Tracks to Spotify
- [x] Story 6.2 — Handle Unmatched Tracks

## Epic 7: Playlist Curation

- [x] Story 7.1 — Review Matched Tracks
- [x] Story 7.2 — Reorder & Remove Tracks
- [x] Story 7.3 — Fetch More Tracks

## Epic 8: Playlist Creation on Spotify

- [x] Story 8.1 — Configure Playlist Details
- [x] Story 8.2 — Create Playlist

## Epic 9: Reliability & Observability

- [ ] Story 9.1 — Handle Partial Failures
- [ ] Story 9.2 — Capture Telemetry

## Epic 10: UX & Usability Improvements

- [x] Story 10.1 — Streamlined Playlist Creation Workflow
- [x] Story 10.2 — Automatic Fetch and Match by Default
- [ ] Story 10.3 — Smart Defaults and Preset Discovery
- [ ] Story 10.4 — Progressive Disclosure of Advanced Options
- [ ] Story 10.5 — Compact Form Layout
- [ ] Story 10.6 — Results Preview and Validation
- [ ] Story 10.7 — Mobile-First Responsive Design
- [ ] Story 10.8 — Clear User Feedback and Error Handling
